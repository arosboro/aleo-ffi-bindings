...
pub trait Network:
    'static + Environment + Copy + Clone + Eq + PartialEq + core::hash::Hash + Send + Sync
{
    /// The network ID.
    const ID: u16;
    /// The network name.
    ...
    const MAX_DATA_SIZE_IN_FIELDS: u32 =
        ((128 * 1024 * 8) / Field::<Self>::SIZE_IN_DATA_BITS) as u32;
    ...
    // fn merkle_tree_bhp<const DEPTH: u8>(leaves: &[Vec<bool>])
